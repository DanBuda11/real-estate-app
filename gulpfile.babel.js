...
let app = {
  ...
  restart: function(event) {
    async.series([app.stop, app.start]);
  }
};
  ...
  async.series(
    {
      checkLocal: function(cb) {
        fs.lstat(path.join(__dirname, 'config', 'local.js'), function(
          err,
          stat
        ) {
          if (err) {
        ...
        .then(function(config) {
          ...
          fs.writeFile(
            ...
            function(err) {
              if (err) {
                cb(
                  'There was a problem saving the local.js config file: ' +
                    err.toString()
                );
              } else {
                cb();
...
gulp.task('cssmin', ['prod-sass-bundle'], function() {
  return gulp
    .src('./public/styles/main.scss.css')
...
gulp.task(
  'deploy',
  require('./gulp/deploy/gulp-deploy')(config, configTemplate, pjson)
);
...
gulp.task('test', function() {
  ...
  return lt
    .execute(
      'gitLatestTag',
      '/usr/share/nginx/apps/test-app',
      'git@bitbucket.org:alarner/impact-web.git'
    )
    .then(data => {
